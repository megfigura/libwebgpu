...
std::vector<ControllerState> Controller::getTickStates(Uint64 frameStart, int nanosPerTick, int numTicks)
{
    std::vector<ControllerState> tickStates(numTicks);
    for (int iTick = 0; iTick < numTicks; iTick++)
    {
        // Actual start/end times
        Uint64 currTickStart = frameStart + (nanosPerTick * iTick);
        Uint64 currTickEnd = currTickStart + nanosPerTick;

        // Acceptable times for events for the current tick -- include any unprocessed events before frame
        Uint64 currTickEventStart = (iTick == 0) ? 0 : currTickStart;
        Uint64 currTickEventEnd = currTickEnd;
